...
Call `list_proposals` (pending). Each row carries `loopSkill` — the loop that raised it — and `evidenceCheck`, a summary of the last time its evidence was verified (`null` means never re-checked since it was written).
...
For each pending proposal, call `verify_proposal`. It re-runs the deterministic evidence checker against the project's CURRENT checkout and records the result, so this is a fact-gathering step: do it for all of them without asking.
